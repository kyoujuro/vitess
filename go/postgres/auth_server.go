...
func isPassScramblePostgresNativePassword(reply, salt []byte, postgresNativePassword string) bool {
	/*
		SERVER:  recv(reply)
	...
	*/
	...
	if postgresNativePassword == "" {
		return false
	}

	if strings.Contains(mysqlNativePassword, "*") {
		postgresNativePassword = postgresNativePassword[1:]
	}

	hash, err := hex.DecodeString(postgresNativePassword)
	if err != nil {
		return false
	}
...
const (
	postgresDialogMessage = "Enter password: "
	...
	// 0x05 - last password
	postgresDialogAskPassword = 0x04
)
...
func authServerDialogSwitchData() []byte {
	result := make([]byte, len(postgresDialogMessage)+2)
	result[0] = postgresDialogAskPassword
	writeNullString(result, 1, postgresDialogMessage)
	return result
}

// AuthServerReadPacketString is a helper method to read a packet
// as a null terminated string. It is used by the postgres_clear_password
// and dialog plugins.
