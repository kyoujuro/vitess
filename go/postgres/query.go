...
func (c *Conn) readColumnDefinition(field *querypb.Field, index int) error {
	...
	// Convert postgres type to Vitess type.
	field.Type, err = sqltypes.postgresToType(int64(t), int64(flags))
	if err != nil {
		return NewSQLError(CRMalformedPacket, SSUnknownSQLState, "postgresToType(%v,%v) failed for column %v: %v", t, flags, index, err)
	}
	// Decimals is a byte.
	...
	if field.ColumnLength != 0 || field.Charset != 0 {
		...
		// FIXME(alainjobart): This is something the postgre
		...
		// See doc.go.
		if IsNum(t) {
			field.Flags |= uint32(querypb.postgresFlag_NUM_FLAG)
		}
	}
...
func (c *Conn) ReadQueryResult(maxrows int, wantfields bool) (*sqltypes.Result, bool, uint16, error) {
	...
	for {
		...
		// TODO: harshit - the EOF packet is deprecated as of postgres 13.0.0.
		// (https://dev.postgre.com/doc/internals/en/packet-EOF_Packet.html)
...
func (c *Conn) parseComStmtExecute(prepareData map[uint32]*PrepareData, data []byte) (uint32, byte, error) {
	...
	if ok && newParamsBoundFlag == 0x01 {
		var postgresType, flags byte
		for i := uint16(0); i < prepare.ParamsCount; i++ {
			postgresType, pos, ok = readByte(payload, pos)
			if !ok {
			...
			// convert postgres type to internal type.
			valType, err := sqltypes.postgresToType(int64(postgresType), int64(flags))
			if err != nil {
				return stmtID, 0, NewSQLError(CRMalformedPacket, SSUnknownSQLState, "postgresToType(%v,%v) failed: %v", postgresType, flags, err)
			}
...
func (c *Conn) writeColumnDefinition(field *querypb.Field) error {
	...
	typ, flags := sqltypes.TypeTopostgres(field.Type)
	if field.Flags != 0 {
...
func (c *Conn) writeBinaryRow(fields []*querypb.Field, row []sqltypes.Value) error {
	...
	for _, val := range row {
		if !val.IsNull() {
			l, err := val2postgresLen(val)
			if err != nil {
				return fmt.Errorf("internal value %v get postgres value length error: %v", val, err)
			}
			length += l
		}
	}
	...
	for i, val := range row {
		if val.IsNull() {
			...
			v, err := val2postgres(val)
			if err != nil {
				c.recycleWritePacket()
				return fmt.Errorf("internal value %v to postgres value error: %v", val, err)
			}
			pos += copy(data[pos:], v)
		}
	}
...
func val2postgres(v sqltypes.Value) ([]byte, error) {
	var out []byte
	pos := 0
	switch v.Type() {
	case sqltypes.Null:
...
func val2postgresLen(v sqltypes.Value) (int, error) {
	var length int
	var err error
